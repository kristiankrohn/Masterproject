    ...
    def mean_and_amplitude(self, m):
        ...
        if self.is_mode_complex:
            ...
            if len(tmin) <= 3:
                ...
                f = splrep(tmin, mmin) #this can cause an error, fixed with a horrible hack
...
# not(np.any(self.mask))
    ...
    def decompose(self):
        ...
        while self.keep_decomposing():
            ...
            #for i in range(self.fixe):
                #print("Problem sentence:")
                #print(self.nbit)
                #print(self.maxiter)
                #print(self.maxiter / 10)
                #print(self.nbit % np.floor(self.maxiter / 10))
                #if (not(self.is_mode_complex)) and (self.nbit > self.maxiter / 5) and
                        #self.nbit % np.floor(self.maxiter / 10) == 0 and not(self.fixe) and self.nbit > 100):
                '''      
                if not self.is_mode_complex:
                    print(self.is_mode_complex)  
                    if (self.nbit > self.maxiter / 5):
                        if self.nbit % np.floor(self.maxiter / 10) == 0:
                            if not(self.fixe) and self.nbit > 100:
